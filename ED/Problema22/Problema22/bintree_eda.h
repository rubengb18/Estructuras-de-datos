...
public:
	...
	//No se por que me decia que no encontraba el identificador de la funcion max
	int max(int a, int b) {
		if (a > b) {
			return a;
		}
	...
	int calcularDiametro(int &alt) {
		if (!empty()) {
			int altIz, altDr;
			auto diamIz = this->left().calcularDiametro(altIz);
			auto diamDr = this->right().calcularDiametro(altDr);
			alt = 1 + max(altIz, altDr);
			return max(max(diamIz, diamDr), 1 + altDr + altIz);
		}
		else {
			alt = 0;
			return 0;
