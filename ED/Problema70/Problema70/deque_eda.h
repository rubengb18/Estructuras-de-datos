...
public:
    ...
    void engordar(deque<int>&aux) {
        bool cambia = false;
        int tam_aux = aux.size();
        Nodo* aux1 = aux.fantasma->sig;

        for (int i = 0; i < tam_aux;i++) {
            if (!cambia) {
                Nodo* aux2 = this->fantasma->sig;
                this->fantasma->sig->ant = aux1;
                this->fantasma->sig = aux1;
                aux1 = aux1->sig;
                this->fantasma->sig->ant = this->fantasma;
                this->fantasma->sig->sig = aux2;
            }
        
            else {
                Nodo* aux2 = this->fantasma->ant;
                this->fantasma->ant->sig = aux1;
                this->fantasma->ant = aux1;
                aux1 = aux1->sig;
                this->fantasma->ant->sig = this->fantasma;
                this->fantasma->ant->ant = aux2;
            }
            cambia = !cambia;
            ++this->nelems;
