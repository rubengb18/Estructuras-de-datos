...
protected:
    void count(int& cont, Link r,const T& lower, const T& upper) const {
        if (r!=nullptr) {
            if ((menor(lower, r->elem) || lower == r->elem) && (menor(r->elem, upper) || upper == r->elem)) {
                cont++;
                count(cont, r->iz, lower, upper);
                count(cont, r->dr, lower, upper);
            }
            else if (menor(upper, r->elem)) {
                count(cont, r->iz, lower, upper);
            }
            else if (menor(r->elem,lower)) {
                count(cont, r->dr, lower, upper);
            }
        }
    }
