...
public:
	...
	bool esArbolBinariodeBusqueda() {
		if (!this->empty()) {
			if (raiz->right != NULL && raiz->left != NULL) {
				if (this->right().root() > raiz->elem && this->left().root() < raiz->elem) {
					return right().esArbolBinariodeBusqueda() && left().esArbolBinariodeBusqueda();
				}
				else {
					return false;
				}
				...
				if (root() < this->right().root()) {
					return right().esArbolBinariodeBusqueda();
				}
				else {
					return false;
				}
				...
				if (root() > this->left().root()) {
					return left().esArbolBinariodeBusqueda();
				}
				else {
					return false;
				}
